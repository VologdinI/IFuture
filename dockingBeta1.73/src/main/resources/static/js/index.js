...
function sendInput() {
    ...
    document.getElementById("divText").innerHTML = '<div id="divTabFolder">\n' +
        '            </div>';
    //тут сделаем грязь с выбором, сервер будет чище
    //снова ip
        let url = "processFrom?goal=" + goal +"&ip=" + ip;
        $.ajax({
            type: "get",
            url: url,
            data: {
...
function reWriteTreePackage() {//только добавил багов, как мне перерисовать
    let usefullStuff= mainUl.getElementById("ulSubFoldersAndFiles1").cloneNode(true);

    for (let i = 0; i < mainUl.childNodes.length; i++) {
        mainUl.childNodes.item(i).remove();
    }
    for (let i = 0; i < usefullStuff.childNodes.length; i++) {
        mainUl.appendChild(usefullStuff.childNodes.item(i));
    }
...
function readALotOfFiles(e) {
    ...
    if(summSize>maxSize)
        ...
        for(let i=0;i<files.length;i++){
            ...
            arPaths.push(files[i].webkitRelativePath);
        }
        for(let i=0;i<files.length;i++){
            arReaders[i].onload = function(e) {
                let contents = e.target.result;
                //let url = "sendFileFromHardDrive?id="+id; //какого черта тут он не меняется?
                $.getJSON('http://www.geoplugin.net/json.gp?jsoncallback=?', function(data) {
                    let url = "sendFileFromHardDrive?id="+id;
                    let ip ="";
                    ip += data.geoplugin_request;
                        $.ajax({
                                type: "post",
