...
function  makeOverlaps() {
    document.getElementById("overlap").setAttribute("disabled", "disabled");
    document.getElementById("loadOver").hidden = false;
    let id = getCurrentTa();
    ...
    if(id!=-1) {
        $.ajax({
            ...
            url: sUrl,
            success: function (data, textStatus) {
                document.getElementById("overlap").removeAttribute("disabled");
                document.getElementById("loadOver").hidden = true;
                if (textStatus == 'success') {
