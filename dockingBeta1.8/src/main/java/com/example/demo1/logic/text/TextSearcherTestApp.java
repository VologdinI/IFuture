...
public class TextSearcherTestApp {
    public static void testTextSearcher(){
        Date start = new Date();
    ...
    public static void testTextEfficientReader() {
        String smallBoyPath = "c:\\Users\\Kano\\Downloads\\LogRepository\\logExample\\BigBoyInside\\SmallBoy.log";
        File file = new File(smallBoyPath);
        TextEfficientReader textEfficientReader = new TextEfficientReader(file);
        for (int i = 0; i < 8; i++) {
            System.out.print(textEfficientReader.getNextBlock());
        }

    }
    public static void main(String[] args) throws IOException {
        testTextEfficientReader();
    }
}
